...
class TradingEnvBenchP(gym.Env):
    ...
    def step(self, action):
        ...
        if self.bench_Q_rem < 0:
            ...
            if self.T_rem <= 1:
                bench_x = self.bench_Q_rem
            self.bench_J = np.exp(-1/self.impact_decay) * self.bench_J + bench_x
            if self.c == 0.5:
                bench_I = self.lamb * np.sign(self.bench_J) * np.sqrt(abs(self.bench_J))
            else:
                bench_I = self.lamb * np.sign(self.bench_J) * abs(self.bench_J) ** self.c
            self.bench_Q_rem -= bench_x
